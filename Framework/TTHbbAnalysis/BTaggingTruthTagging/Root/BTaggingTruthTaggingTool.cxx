...
StatusCode BTaggingTruthTaggingTool::getAllEffMC(){
  ...
  for(unsigned int i=0; i<m_jets->size(); i++){
    eff=1.;
    //m_jets->at(i).print();                                                                                                                                                        
    CorrectionCode corCode=m_effTool->getMCEfficiency(m_jets->at(i).flav, m_jets->at(i).vars, eff);
    if (corCode==CorrectionCode::OutOfValidityRange)  ATH_MSG_WARNING("BTaggingTruthTaggingTool::getAllEffMC:   getMCEfficiency returned CorrectionCode::OutOfValidityRange");
	...
	corCode=m_effTool_allOP[op_appo]->getMCEfficiency(m_jets->at(i).flav, m_jets->at(i).vars, eff_all);
	if (corCode==CorrectionCode::OutOfValidityRange)  ATH_MSG_WARNING("BTaggingTruthTaggingTool::getAllEffMC:   getMCEfficiency returned CorrectionCode::OutOfValidityRange");
...
int BTaggingTruthTaggingTool::ExclusiveExtendedConeHadronFlavourLabel (const xAOD::Jet& jet) {
  // default label means "invalid"
  int label = -1; //see function above
