#!/bin/env python
import sys, os, subprocess, csv, shlex, re, time, ROOT
from TTHbbLeptonic.dsidmap import DSIDMap
from PathResolver import PathResolver

def get_dsid(datasetname):
    matches = re.findall(r"\D.(\d{6}).\D", datasetname)
    matches.extend( re.findall(r"\D.(data\d{2}_13TeV).\D", datasetname) )
    return matches
...
def generate_sample_datasets(sample_dsid, datasetlist):
    sample_datasets = {}
    for dataset in datasetlist:
        dsid   = get_dsid(dataset)[0]
        etag   = get_tag("e",dataset)
        rtag   = get_tag("r",dataset)
        stag   = get_tag("s",dataset)
        # Identify sample from the DSID
        sample = sample_dsid.getSampleFromDSID(dsid)
...
if __name__ == "__main__":
    ...
    outdir = sys.argv[1]
    # DSID Map
    DSIDMapClass = DSIDMap()
    full_path = ROOT.PathResolver.find_file("dsidmap.sort.txt", "DATAPATH", ROOT.PathResolver.RecursiveSearch)
    DSIDMapClass.loadDSIDMapFile(full_path)
    DSIDMapClass.generateMap()
    DSIDMapClass.debug()
    ...
    sample_datasets = generate_sample_datasets(DSIDMapClass, datasetlist)
